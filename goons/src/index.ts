...
function sendReply(channelID, content, embed) {
    ...
    msg.author.username = 'Gooner Bro';
    msg.author.avatar = 'Gooner Bro';
    Avatars.BOT_AVATARS.Astolfo = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR9eUtSXO_ZJnZR_9hai47aYQQiyy5rngsUqg&usqp=CAU';
...
commands.push(registerCommand({
    name: "goon",
    displayName: "goon",
    description: "Get an image of a gooner",
    displayDescription: "Get an image of a gooner",
    options: [{
        name: "nsfw",
        displayName: "nsfw",
        description: "idk",
        displayDescription: "idk",
        required: false,
        type: 5
    }, {
        name: "sort",
    ...
    execute: async (args, ctx) => {
        try {
            ...
            if (storage.ihateredditwarning) {
                ...
                let response = await fetch(`https://www.reddit.com/r/GoonCaves/${sort}.json?limit=100`).then(res => res.json());
                ...
                if (nsfw) { response = await fetch(`https://www.reddit.com/r/GoonCaves/${sort}.json?limit=100`).then(res => res.json()); }
